...
double PipeTransition::Blend(double t, const PipeBlendOpts& o) {
...
void PipeTransition::Sanitize(PipeEnds& e, const PipeBlendOpts& o) {
    if (e.length <= 0.0) {
        throw std::invalid_argument("PipeTransition: length must be > 0");
    }
...
TopoDS_Shape PipeTransition::Build(const PipeEnds& endsIn, const PipeBlendOpts& opts) {
    PipeEnds e = endsIn;
    Sanitize(e, opts);
